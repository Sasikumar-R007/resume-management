...
router.post("/", async (req, res) => {
  try {
    ...
    const recruiterId = `STRC${(count + 1).toString().padStart(3, "0")}`;
    const recruiter = new Recruiter({ ...req.body, recruiterId });
    await recruiter.save();
    res.status(201).json(recruiter);
  } catch (error) {
    console.error("Error saving recruiter:", error);
    res.status(500).json({ message: "Error saving recruiter" });
  }
});
